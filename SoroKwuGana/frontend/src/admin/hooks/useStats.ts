...
export function useToggleComment() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.patch(`/comments/${id}/toggle`).then(r => r.data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['comments'] });
      qc.invalidateQueries({ queryKey: ['comment-stats'] });
      qc.invalidateQueries({ queryKey: ['stats'] });
    },
  });
}
...
export function useCommentStats() {
  return useQuery({
    ...
    queryFn: () => api.get('/comments/stats').then(r => r.data),
    staleTime: 30_000,
  });
}
...
export function useUpdateSecurity() {
  return useMutation({
    mutationFn: (data: { jwtSecret?: string; jwtExpiresIn?: string }) =>
      api.patch('/auth/security', data).then(r => r.data),
  });
}
...
export function useNewsletterStats() {
  return useQuery({
    ...
    queryFn: () => api.get('/newsletter/stats').then(r => r.data),
    staleTime: 30_000,
  });
}

export function useSubscribers(params?: { page?: number; active?: string; search?: string }) {
  return useQuery({
    queryKey: ['subscribers', params],
    queryFn: () => api.get('/newsletter/subscribers', { params }).then(r => r.data),
    staleTime: 15_000,
  });
}
...
export function useDeleteSubscriber() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.delete(`/newsletter/subscribers/${id}`),
    onSuccess: () => {
...
export function useSendNewsletter(postId: string) {
  return useMutation({
    mutationFn: () => api.post(`/newsletter/send/${postId}`).then(r => r.data),
  });
}

export function useNewsletterChart(period: string) {
  return useQuery({
    queryKey: ['newsletter-chart', period],
    queryFn: () => api.get('/newsletter/send-chart', { params: { period } }).then(r => r.data),
    staleTime: 60_000,
  });
}
