  ...
  return (
    <section aria-label="Featured stories" className="max-w-screen-xl mx-auto px-4 lg:px-6 pt-8 pb-12">
      <div className="grid lg:grid-cols-5 gap-4 lg:gap-6">
        ...
        {/* Main hero */}
        <Link to={`/article/${hero.slug}`} className="group lg:col-span-3 relative rounded-3xl overflow-hidden block img-zoom min-h-[280px] sm:min-h-[380px] md:min-h-[420px] lg:min-h-[520px] shadow-2xl">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 lg:p-8">
            ...
            <h1 className="mt-3 font-display font-black text-white text-xl sm:text-2xl lg:text-4xl leading-tight group-hover:text-[#00D4FF] transition-colors line-clamp-3">
              {hero.title}
            </h1>
            <p className="mt-2 text-gray-300 text-sm lg:text-base line-clamp-2 max-w-xl hidden sm:block">{hero.excerpt}</p>
    ...
    <section aria-label="Trending content" className="bg-gray-50 dark:bg-gray-900/50 border-y border-gray-100 dark:border-gray-800 py-12">
      <div className="max-w-screen-xl mx-auto px-4 lg:px-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          {/* Tabs */}
          <div className="flex gap-1 bg-white dark:bg-gray-800 rounded-xl p-1 border border-gray-100 dark:border-gray-700 shadow-sm overflow-x-auto">
            {tabs.map(t => (
              <button key={t.key} onClick={() => setTab(t.key)}
                className={`px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold rounded-lg transition-all whitespace-nowrap ${tab === t.key ? 'bg-gradient-to-r from-[#6C63FF] to-[#FF4D6D] text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}>
                {t.label}
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 md:grid md:grid-cols-2 lg:grid-cols-5 md:overflow-visible snap-x snap-mandatory md:snap-none">
          {trendingPosts.map((post, i) => (
            <Link key={post.id} to={`/article/${post.slug}`} className="group flex gap-3 items-start bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 card-hover shadow-sm min-w-[220px] sm:min-w-[260px] md:min-w-0 snap-start flex-shrink-0 md:flex-shrink">
    ...
    <section className="max-w-screen-xl mx-auto px-4 lg:px-6 py-16">
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#6C63FF] via-[#9C64FF] to-[#FF4D6D] p-10 lg:p-16 text-center text-white shadow-2xl shadow-[#6C63FF]/30">
        ...
        <div className="relative">
          ...
          <h2 className="font-display font-black text-2xl sm:text-3xl lg:text-5xl leading-tight mb-3">
            Stay in the Loop
          </h2>
          <p className="text-white/75 text-base sm:text-lg max-w-md mx-auto mb-8">
