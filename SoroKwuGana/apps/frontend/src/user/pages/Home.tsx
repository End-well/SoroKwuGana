      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-[#6C63FF] to-[#FF4D6D] p-6 text-white shadow-xl shadow-[#6C63FF]/20">
        ...
        <div className="relative">
          ...
          <form className="space-y-3" onSubmit={e => e.preventDefault()}>
            <input type="email" placeholder="your@email.com" required className="w-full bg-white/15 border border-white/25 rounded-xl px-4 py-2.5 text-white placeholder-white/60 text-sm focus:outline-none focus:bg-white/25 transition-colors" />
            <button type="submit" className="w-full bg-white text-[#6C63FF] font-bold py-2.5 rounded-xl text-sm hover:bg-gray-100 transition-colors shadow-lg">Subscribe Free →</button>
          </form>
    ...
    <section className="max-w-screen-xl mx-auto px-4 lg:px-6 py-16">
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#6C63FF] via-[#9C64FF] to-[#FF4D6D] p-10 lg:p-16 text-center text-white shadow-2xl shadow-[#6C63FF]/30">
        ...
        <div className="relative">
          ...
          <p className="text-white/75 text-base sm:text-lg max-w-md mx-auto mb-8">The hottest stories, delivered to your inbox every morning.</p>
          <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto" onSubmit={e => e.preventDefault()}>
            <input type="email" placeholder="Enter your email" required className="flex-1 bg-white/15 border border-white/25 rounded-2xl px-5 py-3.5 text-white placeholder-white/60 focus:outline-none focus:bg-white/25 text-sm" />
            <button type="submit" className="bg-white text-[#6C63FF] font-black px-8 py-3.5 rounded-2xl hover:bg-gray-100 transition-colors text-sm shadow-xl whitespace-nowrap">Get Stories →</button>
          </form>
        </div>
      </div>
    </section>
...
function EntertainmentSpotlight() {
  const { data } = usePosts({ limit: 20 });
  const entPosts = (data?.posts ?? [])
    .filter(p => ['movies', 'music', 'celebrity', 'tv-shows'].includes(p.category?.slug ?? ''))
    .slice(0, 4);

  if (entPosts.length === 0) return null;
  ...
  return (
    <section className="max-w-screen-xl mx-auto px-4 lg:px-6 pb-16">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {entPosts.map(post => <ArticleCard key={post._id} post={post} />)}
      </div>
    </section>
  );
}
